...
# 加载预训练的YOLOv10模型（使用较小模型以减少参数）
model = YOLO('yolov10s.pt')  # 改为 nano 版本，参数更少
...
model.train(
    ...
    epochs=10,  # 改为10轮
    imgsz=320,  # 减少图像大小
    batch=8,   # 减少批次大小以适应CPU
    name='steel_pipe_yolov10_quick',  # 实验名称
...
print(f"快速训练完成！总耗时: {total_time:.2f} 秒 ({total_time/60:.2f} 分钟)")